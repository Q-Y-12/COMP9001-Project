...
##Saving file names as constants, so the names are easier to reuse.
...
def show_logo():
    ...
    print("╚════════════════════════════════════════════╝")
    print()
    ##Using slow_print only for the opening text, so the program still runs quickly.
...
def get_choice(min_num, max_num):
    while True:
        ...
        try:
            ##Converting input into integer because input() always gives a string.
            choice = int(choice)
...
def generate_message_text(situation, style):
    ...
    if situation in templates:
        if style in templates[situation]:
            ##random.choice() picks one message from the list, so it feels less repetitive.
...
def save_to_file(filename, line):
    ##Using append mode, so old records are kept instead of being overwritten.
...
def view_file_as_list(filename, title):
    print()
    print("╭──────────── " + title + " ────────────╮")
    ##Checking the file first, because there may be no saved records yet.
    ...
    for i in range(len(lines)):
        ...
        parts = line.split("|")
        print(str(i + 1) + ". ", end="")
        ##Generated messages have 3 saved parts, so they can be printed more nicely.
        if len(parts) == 3:
...
def add_custom_template():
    ...
    ##Not saving empty templates because they are not useful.
    if text.strip() == "":
...
def find_most_common_from_history(position):
    ...
    for line in lines:
        ...
        ##Ignoring broken lines, so statistics will not crash.
        if len(parts) == 3:
            item = parts[position]
            if item in counts:
...
##Starting the program and keeping the menu running.
def main():
    show_logo()
    while True:
        show_menu()
